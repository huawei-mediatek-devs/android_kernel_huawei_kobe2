...
/* 0~4:af_data 5:af_flag 6~30:sn_data 31:sn_flag 32~47:awb_data 48:awb_flag 49~1916:lsc_data 1917:lsc_flag*/
#define OTP_DATA_SIZE 1918
...
#define  HI556_RET_ERROR     0
#define  HI556_RET_NO_ERROR  1
...
static int read_hi556_module_info(void)
{
	...
	LOG_INF("=== HI556 INFO lens_id=0x%x,vcm_id=0x%x date is %d-%d-%d ===\n",lens_id, vcm_id,year,month,day);
	LOG_INF("=== HI556 INFO check_sum=0x%x,check_sum_cal=0x%x ===\n", check_sum, check_sum_cal);
	if (check_sum == check_sum_cal) {
		return HI556_RET_NO_ERROR;
	}else {
		return HI556_RET_ERROR;
	}
}

static int read_hi556_sn_info(void)
{
	...
	/* lsc group 1 */
	write_cmos_sensor_8(0x010a,((0x041a)>>8)&0xff);
	write_cmos_sensor_8(0x010b,(0x041a)&0xff);
	write_cmos_sensor_8(0x0102,0x01);
	otp_grp_flag = read_cmos_sensor(0x0108);
	LOG_INF("sn_info_grp1_flag = 0x%x\n",otp_grp_flag);
	if (otp_grp_flag == 0x01) {
	...
	if (sn_start_addr != 0) {
		write_cmos_sensor_8(0x010a,((sn_start_addr)>>8)&0xff);
		write_cmos_sensor_8(0x010b,(sn_start_addr)&0xff);
		write_cmos_sensor_8(0x0102,0x01);
		for (i = 0; i < SN_DATA_SIZE + 1; i++)
		{
			hi556_data_sn[i] = read_cmos_sensor(0x0108);
			LOG_INF("hi556_data_sn[%d]=0x%x\n",i,hi556_data_sn[i]);
		}
		for (i = 0; i < SN_DATA_SIZE; i++)
			check_sum_sn_cal += hi556_data_sn[i];

		check_sum_sn_cal += otp_grp_flag;
		LOG_INF("check_sum_sn_cal =0x%x \n",check_sum_sn_cal);
		check_sum_sn = hi556_data_sn[SN_DATA_SIZE];
		check_sum_sn_cal = (check_sum_sn_cal % 255) + 1;
	}
	LOG_INF("=== HI556 check_sum_sn=0x%x, check_sum_sn_cal=0x%x ===\n", check_sum_sn, check_sum_sn_cal);
...
static int read_hi556_awb_data(int awb_start_addr, int otp_grp_flag)
{
...
static int read_hi556_awb_info(void)
{
	...
	write_cmos_sensor_8(0x010a,((0x0469)>>8)&0xff);
	write_cmos_sensor_8(0x010b,(0x0469)&0xff);
	write_cmos_sensor_8(0x0102,0x01);
	otp_grp_flag = read_cmos_sensor(0x0108);
	LOG_INF("awb_info otp_grp_flag = 0x%x\n",otp_grp_flag);
	if (otp_grp_flag == 0x01) {
	...
	if (awb_start_addr != 0)
		ret = read_hi556_awb_data(awb_start_addr, otp_grp_flag);
...
static int read_hi556_lsc_info(void)
{
	...
	/* lsc group 1 */
	write_cmos_sensor_8(0x010a,((0x049d)>>8)&0xff);
	write_cmos_sensor_8(0x010b,(0x049d)&0xff);
	write_cmos_sensor_8(0x0102,0x01);
	otp_grp_flag = read_cmos_sensor(0x0108);
	LOG_INF("lsc_info otp_grp_flag = 0x%x\n",otp_grp_flag);
	if (otp_grp_flag == 0x01) {
	...
	LOG_INF("=== HI556 check_sum_lsc=0x%x, check_sum_lsc_cal=0x%x ===\n", check_sum_lsc, check_sum_lsc_cal);
...
static int read_hi556_af_info(void)
{
	...
	/* lsc group 1 */
	write_cmos_sensor_8(0x010a,((0x1a85)>>8)&0xff);
	write_cmos_sensor_8(0x010b,(0x1a85)&0xff);
	write_cmos_sensor_8(0x0102,0x01);
	otp_grp_flag = read_cmos_sensor(0x0108);
	LOG_INF("af_info_grp1_flag = 0x%x\n",otp_grp_flag);
	if (otp_grp_flag == 0x01) {
	...
	if (af_start_addr != 0) {
		write_cmos_sensor_8(0x010a,((af_start_addr)>>8)&0xff);
		write_cmos_sensor_8(0x010b,(af_start_addr)&0xff);
		write_cmos_sensor_8(0x0102,0x01);
		for (i = 0; i < AF_DATA_SIZE + 1; i++)
		{
			hi556_data_af[i] = read_cmos_sensor(0x0108);
			LOG_INF("hi556_data_af[%d]=0x%x\n",i,hi556_data_af[i]);
		}
		for (i = 0; i < AF_DATA_SIZE; i++)
			check_sum_af_cal += hi556_data_af[i];

		check_sum_af_cal += otp_grp_flag;
		LOG_INF("check_sum_af_cal =0x%x \n",check_sum_af_cal);
		check_sum_af = hi556_data_af[AF_DATA_SIZE];
		check_sum_af_cal = (check_sum_af_cal % 255) + 1;
	}
	LOG_INF("=== HI556 af check_sum=0x%x, check_sum_af_cal=0x%x ===\n", check_sum_af, check_sum_af_cal);
...
static void hi556_sensor_init_settings(void)
{
	/* 1. sensor init */
	...
	write_cmos_sensor(0x0f30, 0x6825); /* Analog PLL setting */
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x095a, 0x492E);
	...
	write_cmos_sensor(0x0006, 0x07C4);
...
//===============================================
		...
		write_cmos_sensor(0x0030, 0x1111);
		write_cmos_sensor(0x0032, 0x1111);
		write_cmos_sensor(0x0006, 0x0f88);
		write_cmos_sensor(0x0a22, 0x0000);
		write_cmos_sensor(0x0a12, 0x0a20);
		write_cmos_sensor(0x0a14, 0x0798);
		write_cmos_sensor(0x003e, 0x0000);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		log_err("Read id fail,sensor id: 0x%x\n", *sensor_id);
