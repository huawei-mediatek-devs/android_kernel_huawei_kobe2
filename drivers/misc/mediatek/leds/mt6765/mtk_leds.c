...
#define BLINK_ON   1
#define BLINK_OFF  0
static unsigned int red_blink_status = BLINK_OFF;
static unsigned int green_blink_status = BLINK_OFF;
/****************************************************************************
 * variables
 ***************************************************************************/
...
static void set_pmic_chrind_blink(const int duty, const int freqsel)
{
	unsigned int val;
	unsigned int ret;
	...
	LEDS_DEBUG("%s: pmic charging status %d, ret = %d\n", __func__, val, ret);
	if (val == 1) { // pmic is charging enabled
		...
		pmic_set_register_value(PMIC_CHRIND_STEP, ISINK_4); // 20mA
		pmic_set_register_value(PMIC_CHRIND_DIM_DUTY, duty);
		pmic_set_register_value(PMIC_CHRIND_DIM_FSEL, freqsel);
		pmic_set_register_value(PMIC_CHRIND_BIAS_EN, NLED_ON);
		pmic_set_register_value(PMIC_CHRIND_CHOP_EN, NLED_ON);
		pmic_set_register_value(PMIC_CHRIND_EN, NLED_ON);
	} else {
		ret = leds_red_ctrl_set(LED_ON);
		if (ret < 0) // gpio set red led fail
			LEDS_DEBUG("%s leds_red_ctrl_set fail\n", __func__);
	}
}
#endif
...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	...
	case MT65XX_LED_PMIC_NLED_ISINK1:
		...
		green_blink_status = BLINK_ON;
		break;
	case MT65XX_LED_PMIC_NLED_ISINK0:
		set_pmic_chrind_blink(duty, pmic_freqsel_array[time_index]);
		red_blink_status = BLINK_ON;
		break;
#endif
	default:
...
#ifdef CONFIG_MTK_PMIC_CHIP_MT6357
static void set_pmic_chrind_led(u32 level)
{
	pmic_set_register_value(PMIC_RG_DRV_128K_CK_PDN, 0x0);
	pmic_set_register_value(PMIC_RG_DRV_CHRIND_CK_PDN, 0);
	pmic_set_register_value(PMIC_CHRIND_MODE_SEL, ISINK_PWM_MODE);
	pmic_set_register_value(PMIC_CHRIND_STEP, ISINK_5); // 24mA
	pmic_set_register_value(PMIC_CHRIND_DIM_DUTY, 255); // full duty
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	if (pmic_type == MT65XX_LED_PMIC_NLED_ISINK1) {
...
#ifdef CONFIG_MTK_PMIC_CHIP_MT6357
		if ((level == BRIGHTNESS_FULL) &&
			(green_blink_status == BLINK_ON)) {
			mutex_unlock(&leds_pmic_mutex);
			return 0;
		}
		green_blink_status = BLINK_OFF;
		pmic_set_register_value(PMIC_RG_DRV_128K_CK_PDN, 0x0);
		pmic_set_register_value(PMIC_RG_DRV_ISINK1_CK_PDN, 0);
		pmic_set_register_value(PMIC_ISINK_CH1_MODE, ISINK_PWM_MODE);
		...
		red_blink_status = BLINK_OFF;
		set_pmic_chrind_led(level);
#endif
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_CUST_BLS_PWM:
		...
		if (panel_info->bl_ic_ctrl_mode == MTK_AAL_I2C_MODE) {
			backlight_level = level * bl_max_level / LCD_KIT_AAL_FULL;
			lcd_kit_bl_ic_set_backlight(backlight_level);
		}
#endif
#endif
	...
	if (panel_info->bl_ic_ctrl_mode == MTK_PWM_HIGH_I2C_MODE) {
		...
		if (led_data->level != level) {
			...
			if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
				...
				backlight_level = (((1 << MT_LED_INTERNAL_LEVEL_BIT_CNT) - 1) *
					pwm_level + LED_HALF) / LED_FULL;
				disp_pq_notify_backlight_changed(backlight_level);
				disp_aal_notify_backlight_changed(backlight_level);
		...
		if (led_data->level != level) {
			...
			if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
				...
				if (led_data->cust.mode == MT65XX_LED_MODE_CUST_BLS_PWM)
					mt_mt65xx_led_set_cust(&led_data->cust, backlight_level);
				else
		...
		if ((led_data->level != level) || (strcmp(led_data->cust.name, "red") == 0)) {
			led_data->level = level;
			if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
				LEDS_DEBUG("Set NLED directly %d at time %lu\n",
					led_data->level, jiffies);
...
#ifdef CONFIG_MTK_AAL_SUPPORT
				if ((level != 0) && (level > bl_max_level))
					pwm_level = bl_max_level;
				else
					pwm_level = level;

				backlight_debug_log(led_data->level, pwm_level);
				backlight_level = pwm_level * LCD_KIT_AAL_FULL / bl_max_level;
				disp_pq_notify_backlight_changed(backlight_level);
				disp_aal_notify_backlight_changed(backlight_level);
#else
				if ((level != 0) && (level > bl_max_level)) {
				...
				if (led_data->level != level) {
					...
					if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
						...
						if ((level != 0) &&
							...
							level = (level * CONFIG_LIGHTNESS_MAPPING_VALUE) / LED_FULL;

						backlight_debug_log(led_data->level, level);
						backlight_level = (((1 << MT_LED_INTERNAL_LEVEL_BIT_CNT) - 1) *
							level + LED_HALF) / LED_FULL;
						disp_pq_notify_backlight_changed(backlight_level);
						if (led_data->cust.mode == MT65XX_LED_MODE_CUST_BLS_PWM)
							mt_mt65xx_led_set_cust(&led_data->cust, backlight_level);
						else
